...
export function set_page_info_cms_plan_info(uuid) {
    if (uuid != null) {
        ...
        // 設置 CKEditor 內容
        if (window.ckeditorEditors && window.ckeditorEditors['#philosophy'] && window.ckeditorEditors['#philosophy'] !== -1) {
            window.ckeditorEditors['#philosophy'].setData(obj_project.philosophy || '');
        } else {
